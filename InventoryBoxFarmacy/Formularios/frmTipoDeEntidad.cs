...
{
    public partial class frmTipoDeEntidad : Form
    {
        public frmTipoDeEntidad()
        {
            InitializeComponent();
        }

        private string NOMBRE_ENTIDAD_PRIVILEGIO = "Tipo de Entidad";
        private string NOMBRE_ENTIDAD = "Administrar los tipos de entidades";
        private string NOMBRE_LLAVE_PRIMARIA = "idTipoDeEntidad";
        ...
        public TipoDeEntidadEN[] oTipoDeEntidad = new TipoDeEntidadEN[0];
        ...
        private DataTable ConvertirClassADT()
        {
            ...
            System.Xml.Serialization.XmlSerializer serializer = new System.Xml.Serialization.XmlSerializer(oTipoDeEntidad.GetType());
            System.IO.StringWriter sw = new System.IO.StringWriter();
            serializer.Serialize(sw, oTipoDeEntidad);
        ...
        private string WhereDinamico() {
            ...
            if (Controles.IsNullOEmptyElControl(chkIdentificador) == false && Controles.IsNullOEmptyElControl(txtIdentificador) == false) {
                Where += string.Format(" and idTipoDeEntidad like '%{0}%' ", txtIdentificador.Text.Trim());
            }

            if (Controles.IsNullOEmptyElControl(chktipoDeEntidad) == false && Controles.IsNullOEmptyElControl(txtDesTipoDeEntidad) == false)
            {
                Where += string.Format(" and DesTipoDeEntidad like '%{0}%' ", txtDesTipoDeEntidad.Text.Trim());
            }
        ...
        private string TituloDinamico()
        {
            ...
            if (Controles.IsNullOEmptyElControl(chktipoDeEntidad) == false && Controles.IsNullOEmptyElControl(txtDesTipoDeEntidad) == false)
            {
                Titulo += string.Format(" Descripción: '{0}', ", txtDesTipoDeEntidad.Text.Trim());
            }
        ...
        private void LLenarListado() {
            ...
            try
            {
                ...
                TipoDeEntidadEN oRegistrosEN = new TipoDeEntidadEN();
                TipoDeEntidadLN oRegistrosLN = new TipoDeEntidadLN();
        ...
        private void FormatearDGV()
        {
            try
            {
                ...
                string OcultarColumnas = "NombreInterno";
                ...
                foreach (DataGridViewColumn c1 in dgvLista.Columns)
                {
                    if (c1.Visible == true)
                    {
                        if (c1.Name.Trim().ToUpper() != "Seleccionar".ToUpper())
                        {
                            ...
                            {
                                oFormato = new FormatoDGV(c1.Name.Trim(), "TipoDeEntidad");
                            }
        ...
        private void MostrarFormularioParaOperacion(string OperacionesARealizar)
        {
            ...
            frmTipoDeEntidadOperacion ofrmTipoDeEntidadOperacion = new frmTipoDeEntidadOperacion();
            ofrmTipoDeEntidadOperacion.OperacionARealizar = OperacionesARealizar;
            ofrmTipoDeEntidadOperacion.NOMBRE_ENTIDAD_PRIVILEGIO = NOMBRE_ENTIDAD_PRIVILEGIO;
            ofrmTipoDeEntidadOperacion.NombreEntidad = NOMBRE_ENTIDAD;
            ofrmTipoDeEntidadOperacion.ValorLlavePrimariaEntidad = this.ValorLlavePrimariaEntidad;
            ofrmTipoDeEntidadOperacion.MdiParent = this.ParentForm;
            ofrmTipoDeEntidadOperacion.Show();
        ...
        private void frmTipoDeEntidad_Shown(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dgvLista.Rows.Count > 0)
                {
                    foreach (DataGridViewRow Fila in dgvLista.Rows)
                    {
                        ...
                        {
                            a++;
                            Array.Resize(ref oTipoDeEntidad, a);

                            oTipoDeEntidad[a - 1] = new TipoDeEntidadEN();
                            oTipoDeEntidad[a - 1].idTipoDeEntidad = Convert.ToInt32(Fila.Cells["idTipoDeEntidad"].Value);
                            oTipoDeEntidad[a - 1].Nombre = Fila.Cells["TipoDeEntidad"].Value.ToString();
                            oTipoDeEntidad[a - 1].NombreInterno = Fila.Cells["NombreInterno"].Value.ToString();
        ...
        private void frmTipoDeEntidad_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void txtDesTipoDeEntidad_KeyUp(object sender, KeyEventArgs e)
        {
            if (Controles.IsNullOEmptyElControl(txtDesTipoDeEntidad))
            {
                chktipoDeEntidad.CheckState = CheckState.Unchecked;
            }
            else { chktipoDeEntidad.CheckState = CheckState.Checked; }

            if (chktipoDeEntidad.CheckState == CheckState.Checked && tsbFiltroAutomatico.CheckState == CheckState.Checked)
            {
                LLenarListado();
            }
        }
