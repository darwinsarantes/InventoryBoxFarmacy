...
namespace InventoryBoxFarmacy.Formularios
{
    public partial class frmUbicacion : Form
    {
        ...
        private string NOMBRE_ENTIDAD_PRIVILEGIO = "Ubicacion";
        private string NOMBRE_ENTIDAD = "Administrar la ubicación de los productos";
        private string NOMBRE_LLAVE_PRIMARIA = "idUbicacion";
        ...
        public UbicacionEN[] oUbicacion = new UbicacionEN[0];
            ...
            try
            {
                ...
                if(oRegistroLN.ListadoParaCombos(oRegistroEN, Program.oDatosDeConexion))
                {
                    if(oRegistroLN.TraerDatos().Rows.Count > 0)
                    {
                        ...
                        {
                            oDatos.Add(Fila["Nombre"].ToString().Trim());
                        }

                        cmbTipoDeUbicacion.AutoCompleteCustomSource = oDatos;
                        cmbTipoDeUbicacion.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
                        cmbTipoDeUbicacion.AutoCompleteSource = AutoCompleteSource.CustomSource;
        ...
        private DataTable ConvertirClassADT()
        {
            ...
            System.Xml.Serialization.XmlSerializer serializer = new System.Xml.Serialization.XmlSerializer(oUbicacion.GetType());
            System.IO.StringWriter sw = new System.IO.StringWriter();
            serializer.Serialize(sw, oUbicacion);
        ...
        private string WhereDinamico() {
            ...
            if (Controles.IsNullOEmptyElControl(chkIdentificador) == false && Controles.IsNullOEmptyElControl(txtIdentificador) == false) {
                Where += string.Format(" and idUbicacion like '%{0}%' ", txtIdentificador.Text.Trim());
            }

            if (Controles.IsNullOEmptyElControl(chkUbicacion) == false && Controles.IsNullOEmptyElControl(txtUbicacion) == false)
            {
                Where += string.Format(" and ub.Nombre like '%{0}%' ", txtUbicacion.Text.Trim());
            }

            if (Controles.IsNullOEmptyElControl(chkTipoDeUbicacion) == false && Controles.IsNullOEmptyElControl(cmbTipoDeUbicacion) == false)
            {
                Where += string.Format(" and tu.idTipoDeUbicacion = {0} ", cmbTipoDeUbicacion.SelectedValue);
            }
        ...
        private string TituloDinamico()
        {
            ...
            if (Controles.IsNullOEmptyElControl(chkUbicacion) == false && Controles.IsNullOEmptyElControl(txtUbicacion) == false)
            {
                Titulo += string.Format(" Ubicación: '{0}', ", txtUbicacion.Text.Trim());
            }

            if (Controles.IsNullOEmptyElControl(chkTipoDeUbicacion) == false && Controles.IsNullOEmptyElControl(cmbTipoDeUbicacion) == false)
            {
                Titulo += string.Format(" Ubicación: '{0}', ", cmbTipoDeUbicacion.Text.Trim());
            }
        ...
        private void FormatearDGV()
        {
            try
            {
                ...
                string OcultarColumnas = "FechaDeModificacion,idUsuarioModificacion,FechaDeCreacion,idUsuarioDeCreacion,idTipoDeUbicacion";
                ...
                foreach (DataGridViewColumn c1 in dgvLista.Columns)
                {
                    if (c1.Visible == true)
                    {
                        if (c1.Name.Trim().ToUpper() != "Seleccionar".ToUpper())
                        {
                            ...
                            {
                                oFormato = new FormatoDGV(c1.Name.Trim(), "Ubicacion");
                            }
        ...
        private void MostrarFormularioParaOperacion(string OperacionesARealizar)
        {
            ...
            frmUbicacionOperacion ofrmUbicacionOperacion = new frmUbicacionOperacion();
            ofrmUbicacionOperacion.OperacionARealizar = OperacionesARealizar;
            ofrmUbicacionOperacion.NOMBRE_ENTIDAD_PRIVILEGIO = NOMBRE_ENTIDAD_PRIVILEGIO;
            ofrmUbicacionOperacion.NombreEntidad = NOMBRE_ENTIDAD;
            ofrmUbicacionOperacion.ValorLlavePrimariaEntidad = this.ValorLlavePrimariaEntidad;
            ofrmUbicacionOperacion.MdiParent = this.ParentForm;
            ofrmUbicacionOperacion.Show();
        ...
        private void frmUbicacion_Shown(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dgvLista.Rows.Count > 0)
                {
                    foreach (DataGridViewRow Fila in dgvLista.Rows)
                    {
                        ...
                        {
                            a++;
                            Array.Resize(ref oUbicacion, a);

                            oUbicacion[a - 1] = new UbicacionEN();
                            oUbicacion[a - 1].idUbicacion = Convert.ToInt32(Fila.Cells["idUbicacion"].Value);
                            oUbicacion[a - 1].Nombre = Fila.Cells["Nombre"].Value.ToString();
                            oUbicacion[a - 1].Descripcion = Fila.Cells["Descripcion"].Value.ToString();
                            oUbicacion[a - 1].oTipoDeUbicacionEN.idTipoDeUbicacion = Convert.ToInt32(Fila.Cells["idTipoDeUbicacion"].Value);
                            oUbicacion[a - 1].oTipoDeUbicacionEN.Nombre = Fila.Cells["TipoDeUbicacion"].Value.ToString();
        ...
        private void frmUbicacion_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void txtDesUbicacion_KeyUp(object sender, KeyEventArgs e)
        {
            if (Controles.IsNullOEmptyElControl(txtUbicacion))
            {
                chkUbicacion.CheckState = CheckState.Unchecked;
            }
            else { chkUbicacion.CheckState = CheckState.Checked; }

            if (chkUbicacion.CheckState == CheckState.Checked && tsbFiltroAutomatico.CheckState == CheckState.Checked)
            {
                LLenarListado();
            }
        }
        ...
        private void cmbTipoDeUbicacion_SelectionChangeCommitted(object sender, EventArgs e)
        {
            chkTipoDeUbicacion.CheckState = CheckState.Checked;
