...
namespace InventoryBoxFarmacy.Formularios
{
    public partial class frmLocacion : Form
    {
        ...
        private string NOMBRE_ENTIDAD_PRIVILEGIO = "Locacion";
        private string NOMBRE_ENTIDAD = "Administrar los diferentes locaciones y espacios donde caeran las secciones del para almacenar el producto";
        private string NOMBRE_LLAVE_PRIMARIA = "idLocacion";
        ...
        public LocacionEN[] oLocacion = new LocacionEN[0];
        ...
        private DataTable ConvertirClassADT()
        {
            ...
            System.Xml.Serialization.XmlSerializer serializer = new System.Xml.Serialization.XmlSerializer(oLocacion.GetType());
            System.IO.StringWriter sw = new System.IO.StringWriter();
            serializer.Serialize(sw, oLocacion);
        ...
        private string WhereDinamico() {
            ...
            if (Controles.IsNullOEmptyElControl(chkIdentificador) == false && Controles.IsNullOEmptyElControl(txtIdentificador) == false) {
                Where += string.Format(" and l.idLocacion like '%{0}%' ", txtIdentificador.Text.Trim());
            }

            if (Controles.IsNullOEmptyElControl(chkCodigo) == false && Controles.IsNullOEmptyElControl(txtCodigo) == false)
            {
                Where += string.Format(" and l.Codigo like '%{0}%' ", txtCodigo.Text.Trim());
            }

            if (Controles.IsNullOEmptyElControl(chkLocacion) == false && Controles.IsNullOEmptyElControl(txtNombre) == false)
            {
                Where += string.Format(" and l.Nombre like '%{0}%' ", txtNombre.Text.Trim());
            }

            if (Controles.IsNullOEmptyElControl(chkCodigoDeAlmacenaje) == false && Controles.IsNullOEmptyElControl(txtCodigoDeAlmacenajae) == false)
            {
                Where += string.Format(" and concat(a.Codigo,'-',b.Codigo,'-',l.Codigo) like '%{0}%' ", txtCodigoDeAlmacenajae.Text.Trim());
            }
        ...
        private string TituloDinamico()
        {
            ...
            if (Controles.IsNullOEmptyElControl(chkLocacion) == false && Controles.IsNullOEmptyElControl(txtNombre) == false)
            {
                Titulo += string.Format(" Locación: '{0}', ", txtNombre.Text.Trim());
            }
        ...
        private void FormatearDGV()
        {
            try
            {
                ...
                string OcultarColumnas = "idBodega,idLocacion,idUsuarioDeCreacion, FechaDeCreacion, idUsuarioModificacion, FechaDeModificacion";
                ...
                foreach (DataGridViewColumn c1 in dgvLista.Columns)
                {
                    if (c1.Visible == true)
                    {
                        if (c1.Name.Trim().ToUpper() != "Seleccionar".ToUpper())
                        {
                            ...
                            {
                                oFormato = new FormatoDGV(c1.Name.Trim(), "Locacion");
                            }
        ...
        private void MostrarFormularioParaOperacion(string OperacionesARealizar)
        {
            ...
            frmLocacionOperacion ofrmLocacionOperacion = new frmLocacionOperacion();
            ofrmLocacionOperacion.OperacionARealizar = OperacionesARealizar;
            ofrmLocacionOperacion.NOMBRE_ENTIDAD_PRIVILEGIO = NOMBRE_ENTIDAD_PRIVILEGIO;
            ofrmLocacionOperacion.NombreEntidad = NOMBRE_ENTIDAD;
            ofrmLocacionOperacion.ValorLlavePrimariaEntidad = this.ValorLlavePrimariaEntidad;
            ofrmLocacionOperacion.MdiParent = this.ParentForm;
            ofrmLocacionOperacion.Show();
        ...
        private void frmLocacion_Shown(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dgvLista.Rows.Count > 0)
                {
                    foreach (DataGridViewRow Fila in dgvLista.Rows)
                    {
                        ...
                        {
                            a++;
                            Array.Resize(ref oLocacion, a);

                            oLocacion[a - 1] = new LocacionEN();
                            oLocacion[a - 1].idLocacion = Convert.ToInt32(Fila.Cells["idLocacion"].Value);
                            oLocacion[a - 1].Nombre = Fila.Cells["Nombre"].Value.ToString();
                            oLocacion[a - 1].Descripcion = Fila.Cells["Descripcion"].Value.ToString();
                            oLocacion[a - 1].Codigo = Fila.Cells["Codigo"].Value.ToString();
        ...
        private void frmLocacion_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void txtDesLocacion_KeyUp(object sender, KeyEventArgs e)
        {
            if (Controles.IsNullOEmptyElControl(txtNombre))
            {
                chkLocacion.CheckState = CheckState.Unchecked;
            }
            else { chkLocacion.CheckState = CheckState.Checked; }
        ...
        private void txtCodigoDeAlmacenajae_KeyUp(object sender, KeyEventArgs e)
        {
            if (Controles.IsNullOEmptyElControl(txtCodigoDeAlmacenajae))
            {
