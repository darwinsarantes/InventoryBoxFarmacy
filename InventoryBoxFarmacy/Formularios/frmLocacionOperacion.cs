...
namespace InventoryBoxFarmacy.Formularios
{
    public partial class frmLocacionOperacion : Form
    {
        ...
        private void DeshabilitarControlesSegunOperacionesARealizar()
        {
            switch (this.OperacionARealizar.ToUpper())
            {
                case "NUEVO":
                    ...
                    txtLDescripcion.Text = string.Empty;
                ...
                case "ELIMINAR":
                    ...
                    txtLDescripcion.ReadOnly = true;
                    txtLLocacion.ReadOnly = true;
                    txtLCodigo.ReadOnly = true;
                    tabControl1.TabPages.Remove(tabControl1.TabPages[0]);
                ...
                case "LOCAL":
                    ...
                    txtIdentificador.ReadOnly = true;
                    txtLCodigo.Text = string.Empty;
                    txtLLocacion.Text = string.Empty;
                    txtLDescripcion.Text = string.Empty;
        ...
        private void Nuevo()
        {
            ...
            CrearColumnasDGVRegistrarContenedores();
        }
        ...
        private void LlenarCamposDesdeBaseDatosSegunID()
        {
            ...
            if (oRegistrosLN.ListadoPorIdentificador(oRegistrosEN, Program.oDatosDeConexion))
            {
                if (oRegistrosLN.TraerDatos().Rows.Count > 0)
                {
                    ...
                    CrearyPoblarColumnasDGVRegistrarContenedor();
        ...
        private bool LosDatosIngresadosSonCorrectos()
        {
            ...
            if (Controles.IsNullOEmptyElControl(txtLocacion))
            {
                EP.SetError(txtLocacion, "Este campo no puede quedar vacío");
                txtLocacion.Focus();
                return false;
            }

            if (Controles.IsNullOEmptyElControl(cmbBodega))
            {
                EP.SetError(cmbBodega, "Se debe seleccionar la bodega");
                cmbBodega.Focus();
                return false;
            }
        ...
        private LocacionEN InformacionDelRegistro() {
            ...
            oRegistroEN.oBodegaEN.idBodega = Convert.ToInt32(cmbBodega.SelectedValue);
            oRegistroEN.oBodegaEN.Nombre = cmbBodega.Text;
        ...
        private LocacionEN InformacionDelRegistroLocal()
        {
            ...
            oRegistroEN.Nombre = txtLLocacion.Text.Trim();
            oRegistroEN.oBodegaEN.idBodega = 0;
                ...
                if (LosDatosIngresadosSonCorrectos())
                {
                    ...
                    if (oRegistroLN.Agregar(oRegistroEN, Program.oDatosDeConexion))
                    {
                        ...
                        if (InsertarActualizarOEliminarRegistros())
                        {
                            ...
                            if (InsertarActualizarOEliminarRegistrosContenedor())
                            {
        ...
        private void tsbNuevo_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ofrmRegistro.NombreEntidad = "Agregar información de la sección";
                ofrmRegistro.ShowDialog();

                if (ofrmRegistro.VariableLocal == true)
                {
                    SeccionEN oRegistroEN = ofrmRegistro.oSeccion;
                    int idLocacion = Convert.ToInt32(txtIdentificador.Text);
                    int idLocacionSeccion = dgvListar.Rows.Count + 1;
                    dgvListar.Rows.Add(false, idLocacionSeccion, 0, idLocacion, oRegistroEN.Codigo, oRegistroEN.Nombre, oRegistroEN.Descripcion, true);
                }
        ...
        private void tsbRegistroLocal_Click(object sender, EventArgs e)
        {
            try
            {
                oLocacion = InformacionDelRegistroLocal();
                VariableLocal = true;
                this.Close();
            }
            catch (Exception ex)
        ...
        private void CrearColumnasDGVRegistrar()
        {
            try
            {
                ...
                string columnas = @" idLocacionSeccion,idSeccion,idLocacion,Codigo,Nombre,Descripcion";
        ...
        private void FormatearDGVRegistros()
        {
            try
            {
                ...
                string OcultarColumnas = "idLocacionSeccion,idLocacion,idSeccion,Actualizar";
            ...
            if (dgvListar.Columns.Count > 0)
            {
                ...
                foreach (DataGridViewColumn c1 in dgvListar.Columns)
                {
                    if (c1.Visible == true)
                    {
                        ...
                        {
                            FormatoDGV oFormato = new FormatoDGV(c1.Name.Trim(), "LocacionSeccion");
        ...
        private string DescripcionDetallaDelRegistro(DataGridView dgv)
        {
            string Mensaje = "";
            ...
            if (dgv.Rows.Count > 0)
            {
                ...
                List<DataGridViewRow> rows = (from item in dgv.Rows.Cast<DataGridViewRow>()
                                              ...
                                              let idSeccion = Convert.ToInt32(item.Cells["idSeccion"].Value)
                                              where Actualizar.Equals(true) && idSeccion == 0
            ...
            {
                Mensaje = string.Format("Información de la secciones a definir: {0}", Mensaje);
            }
        ...
        private DataTable InformacionDeLasSeccionesDentroDeLaLocalizacion()
        {
            ...
            try
            {
                ...
                if (oRegistroLN.ListadoPorIdLocacion(oRegistroEN, Program.oDatosDeConexion))
                {
                ...
                if (DTOrden != null)
                {
                    ...
                    if (DTOrden.Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow row in DTOrden.Rows)
                        {
                            ...
                            int Rows = dgvListar.Rows.Count + 1;
        ...
        private bool InsertarActualizarOEliminarRegistros()
        {
            try
            {
                ...
                else { MessageBox.Show(DescripcionDetallaDelRegistro(dgvListar), "Registros a procesar", MessageBoxButtons.OK, MessageBoxIcon.Information); }
                ...
                if (RowsContacto > 0)
                {
                    ...
                    string NombreLavePrimariaDetalle = "idSeccion";
                    ...
                    while (indice <= dgvListar.Rows.Count - 1)
                    {
                        ...
                        SeccionEN oRegistroEN = InformacionDeLaSeccion(Fila);
                        SeccionLN oRegistroLN = new SeccionLN();
                        ...
                        if (Operacion == "ACTUALIZAR")
                        {
                            ...
                            {
                                this.Cursor = Cursors.Default;
                                DialogResult Respuesta = MessageBox.Show(oRegistroLN.Error + "\n\n¿Desea continuar con el proceso de actualizar para las diferentes secciones restantes?", this.OperacionARealizar, MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation);
                        ...
                        if (Operacion == "ELIMINAR")
                        {
                            ...
                            {
                                this.Cursor = Cursors.Default;
                                DialogResult Respuesta = MessageBox.Show(oRegistroLN.Error + "\n\n¿Desea continuar con el proceso de eliminación para las diferentes secciones restantes?", this.OperacionARealizar, MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation);
                                if (Respuesta == DialogResult.No)
                                {
                        ...
                        if (Operacion == "AGREGAR")
                        {
                            if (oRegistroLN.Agregar(oRegistroEN, Program.oDatosDeConexion))
                            {
                                Fila.Cells[NombreLavePrimariaDetalle].Value = oRegistroEN.idSeccion;
                                int idLocacionSeccion;
                                int.TryParse(Fila.Cells["idLocacionSeccion"].Value.ToString(), out idLocacionSeccion);
                                
                                AgregarValorALAGrilla(idLocacionSeccion, oRegistroEN.idSeccion);
        ...
        private void dgvListar_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                int idSeccion;
                int.TryParse(dgvListar.Rows[e.RowIndex].Cells["idSeccion"].Value.ToString(), out idSeccion);

                if (dgvListar.Rows[e.RowIndex].Cells["idSeccion"].Value == null)
                    return;

                if (idSeccion > 0 && dgvListar.Columns[e.ColumnIndex].Name != "Eliminar")
                {
        ...
        #region "Trabajando con Datagridview de los contenedores"

        private void CrearColumnasDGVRegistrarContenedores()
        {
            try
            {

                string columnas = @"idLocacionSeccion,idContenedor,idSeccion,Codigo,Nombre,Descripcion";

                string[] arrayColumnas = columnas.Split(',');

                dgvListarContenedor.Columns.Clear();
                dgvListarContenedor.ColumnCount = arrayColumnas.Length;

                DataGridViewCheckBoxColumn col1 = new DataGridViewCheckBoxColumn();
                dgvListarContenedor.Columns.Insert(0, col1);
                dgvListarContenedor.Columns[0].Name = "Eliminar";

                int j = 1;
                foreach (string item in arrayColumnas)
                {
                    dgvListarContenedor.Columns[j].Name = item.Trim();
                    j++;
                }

                DataGridViewCheckBoxColumn col2 = new DataGridViewCheckBoxColumn();
                dgvListarContenedor.Columns.Insert(j, col2);
                dgvListarContenedor.Columns[j].Name = "Actualizar";

                FormatearDGVRegistrosDelContenedor();
        ...
        private void FormatearDGVRegistrosDelContenedor()
        {
            try
            {
                this.dgvListarContenedor.AllowUserToResizeRows = false;
                this.dgvListarContenedor.AllowUserToAddRows = false;
                this.dgvListarContenedor.AllowUserToDeleteRows = false;
                this.dgvListarContenedor.DefaultCellStyle.BackColor = Color.White;

                this.dgvListarContenedor.MultiSelect = false;
                this.dgvListarContenedor.RowHeadersVisible = true;

                this.dgvListarContenedor.DefaultCellStyle.Font = new Font("Segoe UI", 8);
                this.dgvListarContenedor.ColumnHeadersDefaultCellStyle.Font = new Font("Segoe UI", 8);
                this.dgvListarContenedor.DefaultCellStyle.SelectionBackColor = Color.LightSteelBlue;
                this.dgvListarContenedor.BackgroundColor = System.Drawing.SystemColors.Window;
                this.dgvListarContenedor.BorderStyle = System.Windows.Forms.BorderStyle.Fixed3D;

                string OcultarColumnas = "idLocacionSeccion,idContenedor,idSeccion,Actualizar";
                OcultarColumnasEnELDGVContenedor(OcultarColumnas);

                FormatearColumnasDGVListarContenedor();

                if (OperacionARealizar == "Eliminar")
                {
                    dgvListarContenedor.DefaultCellStyle.Font = new Font(Font.Name, Font.Size, FontStyle.Strikeout);
                    dgvListarContenedor.DefaultCellStyle.ForeColor = Color.Red;
                    dgvListarContenedor.DefaultCellStyle.SelectionForeColor = Color.Red;
                }
                else
                {
                    dgvListarContenedor.DefaultCellStyle.Font = new Font(Font.Name, Font.Size, FontStyle.Regular);
                    dgvListarContenedor.DefaultCellStyle.ForeColor = Color.Black;
                    dgvListarContenedor.DefaultCellStyle.SelectionForeColor = Color.Black;
                }

                if (OperacionARealizar.ToUpper() == "ELIMINAR".ToUpper() || OperacionARealizar.ToUpper() == "CONSULTAR".ToUpper())
                {
                    dgvListarContenedor.Columns["Eliminar"].ReadOnly = true;
                }

                this.dgvListarContenedor.RowHeadersWidth = 25;

                this.dgvListarContenedor.ColumnHeadersDefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
                this.dgvListarContenedor.SelectionMode = DataGridViewSelectionMode.RowHeaderSelect;
                this.dgvListarContenedor.StandardTab = true;
                this.dgvListarContenedor.ReadOnly = false;
                this.dgvListarContenedor.CellBorderStyle = DataGridViewCellBorderStyle.Single;
        ...
        private void FormatearColumnasDGVListarContenedor()
        {
            if (dgvListarContenedor.Columns.Count > 0)
            {

                foreach (DataGridViewColumn c1 in dgvListarContenedor.Columns)
                {
                    if (c1.Visible == true)
                    {
                        if (c1.Name.Trim().ToUpper() != "Seleccionar".ToUpper())
                        {
                            FormatoDGV oFormato = new FormatoDGV(c1.Name.Trim(), "SeccionContenedor");
                            if (oFormato.ValorEncontrado == false)
                            {
                                oFormato = new FormatoDGV(c1.Name.Trim());
                            }
        ...
        private void OcultarColumnasEnELDGVContenedor(String ColumnasDelDGV)
        {
            if (dgvListarContenedor.Columns.Count > 0)
            {
                if (ColumnasDelDGV.Trim().Length > 0)
        ...
        private DataTable InformacionDeLosContenedoresDentroDeLaSeccion(int idSeccion)
        {
            DataTable ODatos = null;
            try
            {
                ...
                oRegistroEN.Where = string.Format(" and idSeccion = {0}", idSeccion);
                oRegistroEN.OrderBy = " Order by Nombre asc ";
        ...
        private void CrearyPoblarColumnasDGVRegistrarContenedor()
        {
            try
            {
                ...
                if (dgvListar.Rows.Count > 0)
                {
                    foreach (DataGridViewRow Fila in dgvListar.Rows)
                    {
                        ...
                        DataTable DTOrden = InformacionDeLosContenedoresDentroDeLaSeccion(idSeccion);
            ...
            {
                MessageBox.Show(ex.Message, "Crear y Poblar Columnas, con los diferentes contenedor", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private ContenedorEN InformacionDelContenedor(DataGridViewRow Fila)
        {
            ...
            int idContenedor, idSeccion;
            int.TryParse(Fila.Cells["idContenedor"].Value.ToString(), out idContenedor);
            int.TryParse(Fila.Cells["idSeccion"].Value.ToString(), out idSeccion);
            oRegistroEN.idContenedor = idContenedor;
            oRegistroEN.Codigo = Fila.Cells["Codigo"].Value.ToString();
            oRegistroEN.Nombre = Fila.Cells["Nombre"].Value.ToString();
            oRegistroEN.Descripcion = Fila.Cells["Descripcion"].Value.ToString();
            oRegistroEN.oSeccionEN.idSeccion = idSeccion;
            oRegistroEN.oLoginEN = Program.oLoginEN;
            oRegistroEN.idUsuarioDeCreacion = Program.oLoginEN.idUsuario;
            oRegistroEN.idUsuarioModificacion = Program.oLoginEN.idUsuario;
            oRegistroEN.FechaDeCreacion = System.DateTime.Now;
        ...
        private bool LosDatosIngresadosEnGrillaContenedorSonCorrectos(DataGridViewRow Fila)
        {
            try
            {
                if (Convert.ToBoolean(Fila.Cells["Eliminar"].Value) == false)
                {
                    ...
                    {
                        Fila.Selected = true;
                        dgvListarContenedor.CurrentCell = CeldaAEvaluar;
                        dgvListarContenedor.CurrentCell.ErrorText = "Es Necesario agregar un Nombre";
                        MessageBox.Show("Información del Contenedor \n\n Es necesario ingresar un Nombre", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        return false;
                    }
            ...
            {
                Fila.Selected = true;
                dgvListarContenedor.CurrentCell = Fila.Cells["idContenedor"];
                MessageBox.Show("Error al validar datos del Contenedor: " + Fila.Cells["Nombre"].Value.ToString() + "\n" + ex.Message, "Buscar Contenedor", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }

        }
        
        private bool InsertarActualizarOEliminarRegistrosContenedor()
        {
            try
            {
                this.Cursor = Cursors.WaitCursor;

                if (EvaluarDataGridView(dgvListarContenedor) == false)
                {
                    MessageBox.Show("No se encontró registros a procesar", "Evaluación de registros en la lista", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return true;
                }
                else { MessageBox.Show(DescripcionDetallaDelContenedor(dgvListarContenedor), "Registros a procesar", MessageBoxButtons.OK, MessageBoxIcon.Information); }

                int RowsContacto = dgvListarContenedor.Rows.Count;

                if (RowsContacto > 0)
                {                                        
                    int indice = 0;
                    int IndiceProgreso = 0;
                    int TotalDeFilasMarcadasParaEliminar = TotalDeFilasMarcadas(dgvListarContenedor, "Eliminar");
                    //Aqui Volvemos dinamica El codigo poniendo el valor de la llave primaria 
                    string NombreLavePrimariaDetalle = "idContenedor";

                    while (indice <= dgvListarContenedor.Rows.Count - 1)
                    {
                        this.Cursor = Cursors.WaitCursor;
                        
                        DataGridViewRow Fila = dgvListarContenedor.Rows[indice];
                        ...
                        if (LosDatosIngresadosEnGrillaContenedorSonCorrectos(Fila) == false)
                        {
                            if (indice < dgvListarContenedor.Rows.Count - 1)
                            {
                                if (MessageBox.Show("¿Desea continuar con los restantes registros a procesar?", "Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.No)
                                {
                        ...
                        if (Operacion == "ELIMINAR FILA EN GRILLA")
                        {
                            dgvListarContenedor.Rows.Remove(Fila);
                            if (dgvListarContenedor.RowCount <= 0) { indice++; IndiceProgreso++; }
                            continue;
                        }
                        ...
                        if (Operacion == "ACTUALIZAR")
                        {
                            ...
                            {
                                this.Cursor = Cursors.Default;
                                DialogResult Respuesta = MessageBox.Show(oRegistroLN.Error + "\n\n¿Desea continuar con el proceso de acutalizar para los contenedores restantes?", this.OperacionARealizar, MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation);
                                if (Respuesta == DialogResult.No)
                                {
                        ...
                        if (Operacion == "ELIMINAR")
                        {
                            ...
                            {
                                this.Cursor = Cursors.Default;
                                DialogResult Respuesta = MessageBox.Show(oRegistroLN.Error + "\n\n¿Desea continuar con el proceso de eliminación para los contenedores restantes?", this.OperacionARealizar, MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation);
                                if (Respuesta == DialogResult.No)
                                {
                        ...
                        if (Operacion == "ACTUALIZAR")
                        {
                            ...
                            if (oRegistroLN.Actualizar(oRegistroEN, Program.oDatosDeConexion))
                            {
                                ...
                                dgvListarContenedor.Rows[Fila.Index].Cells["Actualizar"].Value = false;
                                oRegistroEN = null;
                                oRegistroLN = null;
                                indice++;
                                IndiceProgreso++;
                        ...
                        if (Operacion == "ELIMINAR")
                        {
                            ...
                            if (oRegistroLN.Eliminar(oRegistroEN, Program.oDatosDeConexion))
                            {
                                ...
                                oRegistroEN = null;
                                oRegistroLN = null;
                                if (dgvListarContenedor.RowCount <= 0) { indice++; }
                                IndiceProgreso++;
                                continue;
        ...
        {
            txtLocacion.Text = txtCodigo.Text;
        }
        ...
        private void tsbGeneradorAutomaticoParaSecciones_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                oFrmAuto.TituloDeLaVentana = "Generar Seccion o fila";
                oFrmAuto.TituloDelGroupBox = "Genera Seccion/Fila automatica";
                oFrmAuto.ShowDialog();
                ...
                if (oFrmAuto.AplicarAutomatico)
                {
                    ...
                    for (int i = oFrmAuto.ValorInicial; i <= oFrmAuto.ValorFinal; i++)
                    {
                        ...
                        int Rows = dgvListar.Rows.Count + 1;
                        dgvListar.Rows.Add(false, Rows, 0, idLocacion, valor, valor, "", true);
                    }
                }

            } catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Generar secciones/filas de manera automatica", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }

        private void txtLCodigo_KeyUp(object sender, KeyEventArgs e)
        {
            txtLLocacion.Text = txtLCodigo.Text;
        }
        ...
        private void tsbAplicarEdicion_Click(object sender, EventArgs e)
        {
            tsbAplicarEdicion.Checked = !tsbAplicarEdicion.Checked;
            bool activarEdicion = true;

            if (tsbAplicarEdicion.Checked == true)
            {
                activarEdicion = false;
            }

            if (dgvListarContenedor.Rows.Count > 0)
            {
                foreach (DataGridViewRow Fila in dgvListarContenedor.Rows)
                {
                    Fila.Cells["Codigo"].ReadOnly = activarEdicion;
                    Fila.Cells["Nombre"].ReadOnly = activarEdicion;
                    Fila.Cells["Descripcion"].ReadOnly = activarEdicion;
        ...
        private void dgvListar_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (DGV.Rows.Count > 0)
            {
                if (DGV.CurrentRow != null)
                {
                    ...
                    int idLocacionSeccion = Convert.ToInt16(Fila.Cells["idLocacionSeccion"].Value.ToString());
                    OcultarRegistrosDVG(dgvListarContenedor);
                    VisualizarRegistroDGV(dgvListarContenedor, "idLocacionSeccion", idLocacionSeccion);
                    
                }
            }
        }

        private void OcultarRegistrosDVG(DataGridView oDGV)
        {
            if (oDGV.Rows.Count > 0)
            {
                foreach (DataGridViewRow Fila in oDGV.Rows)
                {
                    Fila.Visible = false;
                }
            }
        ...
        private void VisualizarRegistroDGV(DataGridView oDGV, string SearchColumnas, int Valor)
        {
            if (oDGV.Rows.Count > 0)
            {
                foreach (DataGridViewRow Fila in oDGV.Rows)
                {
                    if (Convert.ToInt32(Fila.Cells[SearchColumnas].Value.ToString()) == Valor)
                    {
                        Fila.Visible = true;
                    }
                }
        ...
        private void AgregarValorALAGrilla(int ValorSearchColumns, int ValorDeLaColumna)
        {
            if(dgvListarContenedor.Rows.Count > 0)
            {
                foreach (DataGridViewRow Fila in dgvListarContenedor.Rows)
                {
                    int idLocacionSeccion;
                    int.TryParse(Fila.Cells["idLocacionSeccion"].Value.ToString(), out idLocacionSeccion);
                    if (idLocacionSeccion == ValorSearchColumns)
                    {
                        Fila.Cells["idSeccion"].Value = ValorDeLaColumna;
                    }
                }
            }
        }
            ...
            try
            {
                if (dgvListar.Rows.Count > 0)
                {
                    ...
                    if (dgvListar.CurrentRow != null)
                    {
                        ...
                        int idLocacionSeccion = Convert.ToInt16(Fila.Cells["idLocacionSeccion"].Value.ToString());
                        int idSeccion = Convert.ToInt16(Fila.Cells["idSeccion"].Value.ToString());
                        
                        if (ofrmRegistro.VariableLocal == true)
                        {
                            ContenedorEN oRegistroEN = ofrmRegistro.oContenedor;
                            dgvListarContenedor.Rows.Add(false, idLocacionSeccion, 0, idSeccion, oRegistroEN.Codigo, oRegistroEN.Nombre, oRegistroEN.Descripcion, true);
                        }

                    }
                    else
                    {
                        throw new ArgumentException("Se debe de seleccionar la sección en la lista");
                    }

                }
                else
                {
                    throw new ArgumentException("No se han registrado secciones para iniciar este proceso");
                }
            ...
            try
            {
                if (dgvListar.Rows.Count > 0)
                {
                    ...
                    if (dgvListar.CurrentRow != null)
                    {
                        ...
                        frmGenerarSeccionOContenedores oFrmAuto = new frmGenerarSeccionOContenedores();
                        oFrmAuto.StartPosition = FormStartPosition.CenterScreen;
                        oFrmAuto.TituloDeLaVentana = "Generar Contenedor/Casilla";
                        oFrmAuto.TituloDelGroupBox = "Genera Contenedor/Casilla de guardado de producto automatica";
                        oFrmAuto.ShowDialog();
                        ...
                        if (oFrmAuto.AplicarAutomatico)
                        {
                            ...
                            for (int i = oFrmAuto.ValorInicial; i <= oFrmAuto.ValorFinal; i++)
                            {
                                string valor = "";
                                if (i <= 9) { valor = string.Format("0{0}", i.ToString()); } else { valor = i.ToString(); }
                                dgvListarContenedor.Rows.Add(false, idLocacionSeccion, 0, idSeccion, valor, valor, "", true);
                            }
                    ...
                    {
                        throw new ArgumentException("Para iniciar el proceso automatico se debe de seleccionar una elemento en la lista de Secciones");
                    }
        ...
        private void dgvListarContenedor_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dgvListarContenedor.CurrentCell != null && dgvListarContenedor.Columns[dgvListarContenedor.CurrentCell.ColumnIndex].Name == "Eliminar")
            {
                if (Convert.ToBoolean(dgvListarContenedor.Rows[dgvListarContenedor.CurrentCell.RowIndex].Cells["Eliminar"].Value) == true)
                {
                    dgvListarContenedor.Rows[dgvListarContenedor.CurrentCell.RowIndex].DefaultCellStyle.Font = new Font(Font.Name, Font.Size, FontStyle.Strikeout);
                    dgvListarContenedor.Rows[dgvListarContenedor.CurrentCell.RowIndex].DefaultCellStyle.ForeColor = Color.Red;
                    dgvListarContenedor.Rows[dgvListarContenedor.CurrentCell.RowIndex].DefaultCellStyle.SelectionForeColor = Color.Red;
                }
                else
                {
                    dgvListarContenedor.Rows[dgvListarContenedor.CurrentCell.RowIndex].DefaultCellStyle.Font = new Font(Font.Name, Font.Size, FontStyle.Regular);
                    dgvListarContenedor.Rows[dgvListarContenedor.CurrentCell.RowIndex].DefaultCellStyle.ForeColor = Color.Black;
                    dgvListarContenedor.Rows[dgvListarContenedor.CurrentCell.RowIndex].DefaultCellStyle.SelectionForeColor = Color.Black;
                }

            }
        }

        private void dgvListarContenedor_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                int idContenedor;
                int.TryParse(dgvListarContenedor.Rows[e.RowIndex].Cells["idContenedor"].Value.ToString(), out idContenedor);

                if (dgvListarContenedor.Rows[e.RowIndex].Cells["idContenedor"].Value == null)
                    return;

                if (idContenedor > 0 && dgvListarContenedor.Columns[e.ColumnIndex].Name != "Eliminar")
                {
                    dgvListarContenedor.Rows[e.RowIndex].Cells["Actualizar"].Value = true;
                }

            }
            catch (Exception ex)
            {
                MessageBox.Show("Error al ingresar dato en la celda 'dgvLista_CellEndEdit'. \n" + ex.Message, "Listas de contenedores", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void dgvListarContenedor_CurrentCellDirtyStateChanged(object sender, EventArgs e)
        {
            if (dgvListarContenedor.IsCurrentCellDirty)
            {
                dgvListarContenedor.CommitEdit(DataGridViewDataErrorContexts.Commit);
            }
        }

        private void dgvListarContenedor_MouseDown(object sender, MouseEventArgs e)
        {
            try
            {
                if (e.Button == MouseButtons.Right && dgvListarContenedor.CurrentCell != null)
                {
                    //Este código permite seleccionar una fila del DatagridView al presionar click derecho
                    DataGridView.HitTestInfo Hitest = dgvListarContenedor.HitTest(e.X, e.Y);

                    if (Hitest.Type == DataGridViewHitTestType.Cell)
                    {
                        dgvListarContenedor.CurrentCell = dgvListarContenedor.Rows[Hitest.RowIndex].Cells[Hitest.ColumnIndex];
                        dgvListarContenedor.Rows[dgvListarContenedor.CurrentCell.RowIndex].Selected = true;
                    }
        ...
        private void dgvListarContenedor_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                if (dgvListarContenedor.RowCount > 0 && dgvListarContenedor.SelectedRows.Count > 0)
                {
                    if (Convert.ToBoolean(dgvListarContenedor.Rows[dgvListarContenedor.SelectedRows[0].Index].Cells["Eliminar"].Value) == true)
                    {
                        dgvListarContenedor.Rows[dgvListarContenedor.SelectedRows[0].Index].DefaultCellStyle.SelectionForeColor = Color.Red;
                    }
                    else
                    {
                        dgvListarContenedor.Rows[dgvListarContenedor.SelectedRows[0].Index].DefaultCellStyle.SelectionForeColor = Color.Black;
                    }
                }
            }
            catch (Exception ex)
