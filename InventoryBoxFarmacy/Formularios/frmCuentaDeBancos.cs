...
{
    public partial class frmCuentaDeBancos : Form
    {
        public frmCuentaDeBancos()
        {
            InitializeComponent();
        }

        private string NOMBRE_ENTIDAD_PRIVILEGIO = "CuentaDeBancos";
        private string NOMBRE_ENTIDAD = "Administrar la inforamción de los CuentaDeBancos";
        private string NOMBRE_LLAVE_PRIMARIA = "idCuentaDeBancos";
        ...
        public CuentaDeBancosEN[] oCuentaDeBancos = new CuentaDeBancosEN[0];
        ...
        private DataTable ConvertirClassADT()
        {
            ...
            System.Xml.Serialization.XmlSerializer serializer = new System.Xml.Serialization.XmlSerializer(oCuentaDeBancos.GetType());
            System.IO.StringWriter sw = new System.IO.StringWriter();
            serializer.Serialize(sw, oCuentaDeBancos);
        ...
        private string WhereDinamico() {
            ...
            if (Controles.IsNullOEmptyElControl(chkIdentificador) == false && Controles.IsNullOEmptyElControl(txtIdentificador) == false) {
                Where += string.Format(" and idCuentaDeBancos like '%{0}%' ", txtIdentificador.Text.Trim());
            }

            if (Controles.IsNullOEmptyElControl(chkCuentaDeBancos) == false && Controles.IsNullOEmptyElControl(txtNumeroDeCuenta) == false)
            {
                Where += string.Format(" and bn.Nombre like '%{0}%' ", txtNumeroDeCuenta.Text.Trim());
            }
        ...
        private string TituloDinamico()
        {
            ...
            if (Controles.IsNullOEmptyElControl(chkCuentaDeBancos) == false && Controles.IsNullOEmptyElControl(txtNumeroDeCuenta) == false)
            {
                Titulo += string.Format(" Banco: '{0}', ", txtNumeroDeCuenta.Text.Trim());
            }
        ...
        private void LLenarListado() {
            ...
            try
            {
                ...
                CuentaDeBancosEN oRegistrosEN = new CuentaDeBancosEN();
                CuentaDeBancosLN oRegistrosLN = new CuentaDeBancosLN();
                ...
                foreach (DataGridViewColumn c1 in dgvLista.Columns)
                {
                    if (c1.Visible == true)
                    {
                        if (c1.Name.Trim().ToUpper() != "Seleccionar".ToUpper())
                        {
                            ...
                            {
                                oFormato = new FormatoDGV(c1.Name.Trim(), "CuentaDeBancos");
                            }
        ...
        {
            /*
            frmCuentaDeBancosOperacion ofrmCuentaDeBancosOperacion = new frmCuentaDeBancosOperacion();
            ofrmCuentaDeBancosOperacion.OperacionARealizar = OperacionesARealizar;
            ofrmCuentaDeBancosOperacion.NOMBRE_ENTIDAD_PRIVILEGIO = NOMBRE_ENTIDAD_PRIVILEGIO;
            ofrmCuentaDeBancosOperacion.NombreEntidad = NOMBRE_ENTIDAD;
            ofrmCuentaDeBancosOperacion.ValorLlavePrimariaEntidad = this.ValorLlavePrimariaEntidad;
            ofrmCuentaDeBancosOperacion.MdiParent = this.ParentForm;
            ofrmCuentaDeBancosOperacion.Show();*/
        ...
        private void frmCuentaDeBancos_Shown(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dgvLista.Rows.Count > 0)
                {
                    foreach (DataGridViewRow Fila in dgvLista.Rows)
                    {
                        ...
                        {
                            a++;
                            Array.Resize(ref oCuentaDeBancos, a);

                            oCuentaDeBancos[a - 1] = new CuentaDeBancosEN();
                            oCuentaDeBancos[a - 1].idCuentaDeBancos = Convert.ToInt32(Fila.Cells["idCuentaDeBancos"].Value);
                            oCuentaDeBancos[a - 1].NumeroDeCuenta = Fila.Cells["NumeroDeCuenta"].Value.ToString();
                            oCuentaDeBancos[a - 1].Descripcion = Fila.Cells["Descripcion"].Value.ToString();
                            oCuentaDeBancos[a - 1].oBancosEN.idBancos = Convert.ToInt32(Fila.Cells["idBancos"].Value.ToString());
        ...
        private void frmCuentaDeBancos_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void txtDesCuentaDeBancos_KeyUp(object sender, KeyEventArgs e)
        {
            if (Controles.IsNullOEmptyElControl(txtNumeroDeCuenta))
            {
                chkCuentaDeBancos.CheckState = CheckState.Unchecked;
            }
            else { chkCuentaDeBancos.CheckState = CheckState.Checked; }

            if (chkCuentaDeBancos.CheckState == CheckState.Checked && tsbFiltroAutomatico.CheckState == CheckState.Checked)
            {
                LLenarListado();
            }
        }
