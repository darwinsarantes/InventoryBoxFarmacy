...
namespace InventoryBoxFarmacy.Formularios
{
    public partial class frmBancos : Form
    {
        ...
        private string NOMBRE_ENTIDAD_PRIVILEGIO = "Bancos";
        private string NOMBRE_ENTIDAD = "Administrar la inforamción de los bancos";
        private string NOMBRE_LLAVE_PRIMARIA = "idBancos";
        ...
        public BancosEN[] oBancos = new BancosEN[0];
        ...
        private DataTable ConvertirClassADT()
        {
            ...
            System.Xml.Serialization.XmlSerializer serializer = new System.Xml.Serialization.XmlSerializer(oBancos.GetType());
            System.IO.StringWriter sw = new System.IO.StringWriter();
            serializer.Serialize(sw, oBancos);
        ...
        private string WhereDinamico() {
            ...
            if (Controles.IsNullOEmptyElControl(chkIdentificador) == false && Controles.IsNullOEmptyElControl(txtIdentificador) == false) {
                Where += string.Format(" and idBancos like '%{0}%' ", txtIdentificador.Text.Trim());
            }

            if (Controles.IsNullOEmptyElControl(chkBancos) == false && Controles.IsNullOEmptyElControl(txtNombre) == false)
            {
                Where += string.Format(" and bn.Nombre like '%{0}%' ", txtNombre.Text.Trim());
            }
        ...
        private string TituloDinamico()
        {
            ...
            if (Controles.IsNullOEmptyElControl(chkBancos) == false && Controles.IsNullOEmptyElControl(txtNombre) == false)
            {
                Titulo += string.Format(" Banco: '{0}', ", txtNombre.Text.Trim());
            }
        ...
        private void LLenarListado() {
            ...
            try
            {
                ...
                BancosEN oRegistrosEN = new BancosEN();
                BancosLN oRegistrosLN = new BancosLN();
                ...
                foreach (DataGridViewColumn c1 in dgvLista.Columns)
                {
                    if (c1.Visible == true)
                    {
                        if (c1.Name.Trim().ToUpper() != "Seleccionar".ToUpper())
                        {
                            ...
                            {
                                oFormato = new FormatoDGV(c1.Name.Trim(), "Bancos");
                            }
        ...
        private void MostrarFormularioParaOperacion(string OperacionesARealizar)
        {
            ...
            frmBancosOperacion ofrmBancosOperacion = new frmBancosOperacion();
            ofrmBancosOperacion.OperacionARealizar = OperacionesARealizar;
            ofrmBancosOperacion.NOMBRE_ENTIDAD_PRIVILEGIO = NOMBRE_ENTIDAD_PRIVILEGIO;
            ofrmBancosOperacion.NombreEntidad = NOMBRE_ENTIDAD;
            ofrmBancosOperacion.ValorLlavePrimariaEntidad = this.ValorLlavePrimariaEntidad;
            ofrmBancosOperacion.MdiParent = this.ParentForm;
            ofrmBancosOperacion.Show();
            ...
            try
            {
                ...
                if (dgvLista.Rows.Count > 0)
                {
                    foreach (DataGridViewRow Fila in dgvLista.Rows)
                    {
                        ...
                        {
                            a++;
                            Array.Resize(ref oBancos, a);

                            oBancos[a - 1] = new BancosEN();
                            oBancos[a - 1].idBancos = Convert.ToInt32(Fila.Cells["idBancos"].Value);
                            oBancos[a - 1].Nombre = Fila.Cells["Nombre"].Value.ToString();
                            oBancos[a - 1].Descripcion = Fila.Cells["Descripcion"].Value.ToString();
        ...
        private void txtDesBancos_KeyUp(object sender, KeyEventArgs e)
        {
            if (Controles.IsNullOEmptyElControl(txtNombre))
            {
                chkBancos.CheckState = CheckState.Unchecked;
            }
            else { chkBancos.CheckState = CheckState.Checked; }

            if (chkBancos.CheckState == CheckState.Checked && tsbFiltroAutomatico.CheckState == CheckState.Checked)
            {
                LLenarListado();
            }
        }
