...
{
    public partial class frmTiposDeEntradas : Form
    {
        public frmTiposDeEntradas()
        {
            InitializeComponent();
        }

        private string NOMBRE_ENTIDAD_PRIVILEGIO = "Tipo de Entrada";
        private string NOMBRE_ENTIDAD = "Administrar los diferentes tipos de entrada";
        private string NOMBRE_LLAVE_PRIMARIA = "idTipoDeEntrada";
        ...
        public TiposDeEntradasEN[] oTiposDeEntradas = new TiposDeEntradasEN[0];
        ...
        private DataTable ConvertirClassADT()
        {
            ...
            System.Xml.Serialization.XmlSerializer serializer = new System.Xml.Serialization.XmlSerializer(oTiposDeEntradas.GetType());
            System.IO.StringWriter sw = new System.IO.StringWriter();
            serializer.Serialize(sw, oTiposDeEntradas);
        ...
        private string WhereDinamico() {
            ...
            if (Controles.IsNullOEmptyElControl(chkIdentificador) == false && Controles.IsNullOEmptyElControl(txtIdentificador) == false) {
                Where += string.Format(" and idTipoDeEntrada like '%{0}%' ", txtIdentificador.Text.Trim());
            }

            if (Controles.IsNullOEmptyElControl(chkTiposDeEntradas) == false && Controles.IsNullOEmptyElControl(txtTipoDeEntrada) == false)
            {
                Where += string.Format(" and te.Nombre like '%{0}%' ", txtTipoDeEntrada.Text.Trim());
            }
        ...
        private string TituloDinamico()
        {
            ...
            if (Controles.IsNullOEmptyElControl(chkTiposDeEntradas) == false && Controles.IsNullOEmptyElControl(txtTipoDeEntrada) == false)
            {
                Titulo += string.Format(" Tipo de entrada: '{0}', ", txtTipoDeEntrada.Text.Trim());
            }
        ...
        private void LLenarListado() {
            ...
            try
            {
                ...
                TiposDeEntradasEN oRegistrosEN = new TiposDeEntradasEN();
                TiposDeEntradasLN oRegistrosLN = new TiposDeEntradasLN();
                ...
                foreach (DataGridViewColumn c1 in dgvLista.Columns)
                {
                    if (c1.Visible == true)
                    {
                        if (c1.Name.Trim().ToUpper() != "Seleccionar".ToUpper())
                        {
                            ...
                            {
                                oFormato = new FormatoDGV(c1.Name.Trim(), "TiposDeEntradas");
                            }
        ...
        private void MostrarFormularioParaOperacion(string OperacionesARealizar)
        {
            ...
            frmTiposDeEntradasOperacion ofrmTiposDeEntradasOperacion = new frmTiposDeEntradasOperacion();
            ofrmTiposDeEntradasOperacion.OperacionARealizar = OperacionesARealizar;
            ofrmTiposDeEntradasOperacion.NOMBRE_ENTIDAD_PRIVILEGIO = NOMBRE_ENTIDAD_PRIVILEGIO;
            ofrmTiposDeEntradasOperacion.NombreEntidad = NOMBRE_ENTIDAD;
            ofrmTiposDeEntradasOperacion.ValorLlavePrimariaEntidad = this.ValorLlavePrimariaEntidad;
            ofrmTiposDeEntradasOperacion.MdiParent = this.ParentForm;
            ofrmTiposDeEntradasOperacion.Show();
        ...
        private void frmTiposDeEntradas_Shown(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dgvLista.Rows.Count > 0)
                {
                    foreach (DataGridViewRow Fila in dgvLista.Rows)
                    {
                        ...
                        {
                            a++;
                            Array.Resize(ref oTiposDeEntradas, a);

                            oTiposDeEntradas[a - 1] = new TiposDeEntradasEN();
                            oTiposDeEntradas[a - 1].idTipoDeEntrada = Convert.ToInt32(Fila.Cells["idTipoDeEntrada"].Value);
                            oTiposDeEntradas[a - 1].Nombre = Fila.Cells["Nombre"].Value.ToString();
                            oTiposDeEntradas[a - 1].Descripcion = Fila.Cells["Descripcion"].Value.ToString();
        ...
        private void frmTiposDeEntradas_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void txtDesTiposDeEntradas_KeyUp(object sender, KeyEventArgs e)
        {
            if (Controles.IsNullOEmptyElControl(txtTipoDeEntrada))
            {
                chkTiposDeEntradas.CheckState = CheckState.Unchecked;
            }
            else { chkTiposDeEntradas.CheckState = CheckState.Checked; }

            if (chkTiposDeEntradas.CheckState == CheckState.Checked && tsbFiltroAutomatico.CheckState == CheckState.Checked)
            {
                LLenarListado();
            }
        }
