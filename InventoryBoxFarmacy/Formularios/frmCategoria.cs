...
namespace InventoryBoxFarmacy.Formularios
{
    public partial class frmCategoria : Form
    {
        ...
        private string NOMBRE_ENTIDAD_PRIVILEGIO = "Categoria";
        private string NOMBRE_ENTIDAD = "Administrar la información de las categorias de los productos";
        private string NOMBRE_LLAVE_PRIMARIA = "idCategoria";
        ...
        public CategoriaEN[] oCategoria = new CategoriaEN[0];
        ...
        private DataTable ConvertirClassADT()
        {
            ...
            System.Xml.Serialization.XmlSerializer serializer = new System.Xml.Serialization.XmlSerializer(oCategoria.GetType());
            System.IO.StringWriter sw = new System.IO.StringWriter();
            serializer.Serialize(sw, oCategoria);
        ...
        private string WhereDinamico() {
            ...
            if (Controles.IsNullOEmptyElControl(chkIdentificador) == false && Controles.IsNullOEmptyElControl(txtIdentificador) == false) {
                Where += string.Format(" and idCategoria like '%{0}%' ", txtIdentificador.Text.Trim());
            }

            if (Controles.IsNullOEmptyElControl(chkCategoria) == false && Controles.IsNullOEmptyElControl(txtCategoria) == false)
            {
                Where += string.Format(" and ub.Nombre like '%{0}%' ", txtCategoria.Text.Trim());
            }
        ...
        private string TituloDinamico()
        {
            ...
            if (Controles.IsNullOEmptyElControl(chkCategoria) == false && Controles.IsNullOEmptyElControl(txtCategoria) == false)
            {
                Titulo += string.Format(" Descripción: '{0}', ", txtCategoria.Text.Trim());
            }
                ...
                foreach (DataGridViewColumn c1 in dgvLista.Columns)
                {
                    if (c1.Visible == true)
                    {
                        if (c1.Name.Trim().ToUpper() != "Seleccionar".ToUpper())
                        {
                            ...
                            {
                                oFormato = new FormatoDGV(c1.Name.Trim(), "Categoria");
                            }
        ...
        private void MostrarFormularioParaOperacion(string OperacionesARealizar)
        {
            ...
            frmCategoriaOperacion ofrmCategoriaOperacion = new frmCategoriaOperacion();
            ofrmCategoriaOperacion.OperacionARealizar = OperacionesARealizar;
            ofrmCategoriaOperacion.NOMBRE_ENTIDAD_PRIVILEGIO = NOMBRE_ENTIDAD_PRIVILEGIO;
            ofrmCategoriaOperacion.NombreEntidad = NOMBRE_ENTIDAD;
            ofrmCategoriaOperacion.ValorLlavePrimariaEntidad = this.ValorLlavePrimariaEntidad;
            ofrmCategoriaOperacion.MdiParent = this.ParentForm;
            ofrmCategoriaOperacion.Show();
        ...
        private void frmCategoria_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void txtDesCategoria_KeyUp(object sender, KeyEventArgs e)
        {
            if (Controles.IsNullOEmptyElControl(txtCategoria))
            {
                chkCategoria.CheckState = CheckState.Unchecked;
            }
            else { chkCategoria.CheckState = CheckState.Checked; }

            if (chkCategoria.CheckState == CheckState.Checked && tsbFiltroAutomatico.CheckState == CheckState.Checked)
            {
                LLenarListado();
            }
        }
