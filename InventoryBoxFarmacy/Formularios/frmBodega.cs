...
namespace InventoryBoxFarmacy.Formularios
{
    public partial class frmBodega : Form
    {
        ...
        private string NOMBRE_ENTIDAD = "Administrar los diferentes Bodegaes y espacios donde caeran las secciones del para almacenar el producto";
        ...
        public BodegaEN[] oBodega = new BodegaEN[0];
        ...
        private DataTable ConvertirClassADT()
        {
            ...
            System.Xml.Serialization.XmlSerializer serializer = new System.Xml.Serialization.XmlSerializer(oBodega.GetType());
            System.IO.StringWriter sw = new System.IO.StringWriter();
            serializer.Serialize(sw, oBodega);
        ...
        private void MostrarFormularioParaOperacion(string OperacionesARealizar)
        {
            ...
            frmBodegaOperacion ofrmBodegaOperacion = new frmBodegaOperacion();
            ofrmBodegaOperacion.OperacionARealizar = OperacionesARealizar;
            ofrmBodegaOperacion.NOMBRE_ENTIDAD_PRIVILEGIO = NOMBRE_ENTIDAD_PRIVILEGIO;
            ofrmBodegaOperacion.NombreEntidad = NOMBRE_ENTIDAD;
            ofrmBodegaOperacion.ValorLlavePrimariaEntidad = this.ValorLlavePrimariaEntidad;
            ofrmBodegaOperacion.MdiParent = this.ParentForm;
            ofrmBodegaOperacion.Show();
            ...
            try
            {
                ...
                if (dgvLista.Rows.Count > 0)
                {
                    foreach (DataGridViewRow Fila in dgvLista.Rows)
                    {
                        ...
                        {
                            a++;
                            Array.Resize(ref oBodega, a);

                            oBodega[a - 1] = new BodegaEN();
                            oBodega[a - 1].idBodega = Convert.ToInt32(Fila.Cells["idBodega"].Value);
                            oBodega[a - 1].Nombre = Fila.Cells["Nombre"].Value.ToString();
                            oBodega[a - 1].Descripcion = Fila.Cells["Descripcion"].Value.ToString();
                            oBodega[a - 1].Codigo = Fila.Cells["Codigo"].Value.ToString();
        ...
        private void frmBodega_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void txtDesBodega_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            else { chkBodega.CheckState = CheckState.Checked; }
