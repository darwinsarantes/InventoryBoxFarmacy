...
{
    public partial class frmTipoDeSalida : Form
    {
        public frmTipoDeSalida()
        {
            InitializeComponent();
        }

        private string NOMBRE_ENTIDAD_PRIVILEGIO = "Tipo de Salida";
        private string NOMBRE_ENTIDAD = "Administrar los diferentes tipos de salidas";
        private string NOMBRE_LLAVE_PRIMARIA = "idTipoDeSalida";
        ...
        public TipoDeSalidaEN[] oTipoDeSalida = new TipoDeSalidaEN[0];
        ...
        private DataTable ConvertirClassADT()
        {
            ...
            System.Xml.Serialization.XmlSerializer serializer = new System.Xml.Serialization.XmlSerializer(oTipoDeSalida.GetType());
            System.IO.StringWriter sw = new System.IO.StringWriter();
            serializer.Serialize(sw, oTipoDeSalida);
        ...
        private string WhereDinamico() {
            ...
            if (Controles.IsNullOEmptyElControl(chkIdentificador) == false && Controles.IsNullOEmptyElControl(txtIdentificador) == false) {
                Where += string.Format(" and idTipoDeSalida like '%{0}%' ", txtIdentificador.Text.Trim());
            }
        ...
        private string TituloDinamico()
        {
            ...
            if (Controles.IsNullOEmptyElControl(chkTipoDeSalida) == false && Controles.IsNullOEmptyElControl(txtNombre) == false)
            {
                Titulo += string.Format(" Tipo de Salida: '{0}', ", txtNombre.Text.Trim());
            }
        ...
        private void LLenarListado() {
            ...
            try
            {
                ...
                TipoDeSalidaEN oRegistrosEN = new TipoDeSalidaEN();
                TipoDeSalidaLN oRegistrosLN = new TipoDeSalidaLN();
                ...
                foreach (DataGridViewColumn c1 in dgvLista.Columns)
                {
                    if (c1.Visible == true)
                    {
                        if (c1.Name.Trim().ToUpper() != "Seleccionar".ToUpper())
                        {
                            ...
                            {
                                oFormato = new FormatoDGV(c1.Name.Trim(), "TipoDeSalida");
                            }
        ...
        private void MostrarFormularioParaOperacion(string OperacionesARealizar)
        {
            ...
            frmTipoDeSalidaOperacion ofrmTipoDeSalidaOperacion = new frmTipoDeSalidaOperacion();
            ofrmTipoDeSalidaOperacion.OperacionARealizar = OperacionesARealizar;
            ofrmTipoDeSalidaOperacion.NOMBRE_ENTIDAD_PRIVILEGIO = NOMBRE_ENTIDAD_PRIVILEGIO;
            ofrmTipoDeSalidaOperacion.NombreEntidad = NOMBRE_ENTIDAD;
            ofrmTipoDeSalidaOperacion.ValorLlavePrimariaEntidad = this.ValorLlavePrimariaEntidad;
            ofrmTipoDeSalidaOperacion.MdiParent = this.ParentForm;
            ofrmTipoDeSalidaOperacion.Show();
        ...
        private void frmTipoDeSalida_Shown(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dgvLista.Rows.Count > 0)
                {
                    foreach (DataGridViewRow Fila in dgvLista.Rows)
                    {
                        ...
                        {
                            a++;
                            Array.Resize(ref oTipoDeSalida, a);

                            oTipoDeSalida[a - 1] = new TipoDeSalidaEN();
                            oTipoDeSalida[a - 1].idTipoDeSalida = Convert.ToInt32(Fila.Cells["idTipoDeSalida"].Value);
                            oTipoDeSalida[a - 1].Nombre = Fila.Cells["Nombre"].Value.ToString();
                            oTipoDeSalida[a - 1].Descripcion = Fila.Cells["Descripcion"].Value.ToString();
        ...
        private void frmTipoDeSalida_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void txtDesTipoDeSalida_KeyUp(object sender, KeyEventArgs e)
        {
            if (Controles.IsNullOEmptyElControl(txtNombre))
            {
                chkTipoDeSalida.CheckState = CheckState.Unchecked;
            }
            else { chkTipoDeSalida.CheckState = CheckState.Checked; }

            if (chkTipoDeSalida.CheckState == CheckState.Checked && tsbFiltroAutomatico.CheckState == CheckState.Checked)
            {
                LLenarListado();
            }
        }
